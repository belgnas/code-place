...
private:
  ...
  string pick_add; // 取件地址（默认为哈尔滨理工大学）
  int weight;       // 重量
  int price;        // 价格
...
{
  name = "张三";
  basic_address = "哈尔滨理工大学";
  contact_information = "10000000";
  password = 1111111; // 默认密码
}
...
void Client::add_pack(int k) // 添加包裹
{
  ...
  getline(cin, c);
  cout << endl;
  cout << "请输入您预订的上门取货时间：" << endl;
  getline(cin, d);
  cout << endl;
  cout << "请输入您预定的取货地址：" << endl;
  getline(cin, e);
  cout << endl;
...
int main() // 主函数
{
  ...
  obj_staff[0].staff_password = "123456"; // 默认密码
  obj_staff[1].staff_password = "123456";
  cout << "------------------------------------------------------------------------" << endl;
  cout << "欢迎来到“新生入学快递护航”物流系统" << endl;
  cout << endl;
  cout << "请选择您想要执行的操作:" << endl;
  cout << endl;
